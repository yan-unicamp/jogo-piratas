...
public class PersonagemFactory {
    private static Aliado adicionarHabilidadesDeNivel(Aliado a) {
        if (!a.temHabilidadeDesbloqueavelNoNivel(3))
            a.adicionarHabilidadeDesbloqueavel(3, () -> HabilidadeFactory.criarGolpeConcentrado(a));
        if (!a.temHabilidadeDesbloqueavelNoNivel(5))
            a.adicionarHabilidadeDesbloqueavel(5, () -> HabilidadeFactory.criarPosturaDefensiva(a));
        if (!a.temHabilidadeDesbloqueavelNoNivel(7))
            a.adicionarHabilidadeDesbloqueavel(7, () -> HabilidadeFactory.criarFolegoExtra(a));
        if (!a.temHabilidadeDesbloqueavelNoNivel(10))
            a.adicionarHabilidadeDesbloqueavel(10, () -> HabilidadeFactory.criarAtaqueSupremo(a));
        return a;
    }

    public static Aliado criarLuffy() {
        Aliado luffy = new Aliado("Luffy", 100, 0.9f, 10, 1, 0, "luffy.png"); // nome, vida, defesa, iniciativa, nivel,
                                                                              // xp
    ...
    public static java.util.ArrayList<Inimigo> sortearInimigosCanonicos(int capitulo, int nivel) {
        ...
        if (capitulo == 1) {
            if (sorteio == 0)
                inimigos.add(criarBuggy(nivel));
            else if (sorteio == 1)
                inimigos.add(criarDonKrieg(nivel));
            else
                inimigos.add(criarArlong(nivel));
        } else if (capitulo == 2) {
            if (sorteio == 0)
                inimigos.add(criarWapol(nivel));
            else if (sorteio == 1)
                inimigos.add(criarEneru(nivel));
            else
                inimigos.add(criarMoria(nivel));
        } else {
            if (sorteio == 0)
                inimigos.add(criarMagellan(nivel));
            else if (sorteio == 1)
                inimigos.add(criarDoflamingo(nivel));
            else
                inimigos.add(criarBigMom(nivel));
        }
        return inimigos;
    }
    ...
    public static Inimigo criarMiniBoss(String nome, int idx, String spriteKey, String habNome, int habPoder) {
        ...
        e.adicionarHabilidade(
                new entidades.Habilidade(habNome, entidades.TipoHabilidade.DANO, (habPoder / 2) + (idx * 2)));
        return e;
    }

    public static Inimigo criarBoss(String nome, int idx, String spriteKey, String hab1, int pod1, String hab2,
            int pod2) {
        int hp = 120 + idx * 60;
        ...
        b.adicionarHabilidade(new entidades.Habilidade(hab1, entidades.TipoHabilidade.DANO, (pod1 / 2) + (idx * 3)));
        b.adicionarHabilidade(new entidades.Habilidade(hab2, entidades.TipoHabilidade.DANO, (pod2 / 2) + (idx * 4)));
        return b;
    }
}
